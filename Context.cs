...
namespace Toolbox
{
	...
	public static class Context<TypeT>
	{
		...
		{
			readonly Stack<ContextStackEntry<TypeT>> Stack = new Stack<ContextStackEntry<TypeT>>();
			...
			{
				Stack.Push(new ContextStackEntry<TypeT> { Value = value, OnPopped = onPopped });
				return PopAction;
			}
			...
			void pop()
			{
				...
				ContextStackEntry<TypeT> e = Stack.Pop();
				if (e.OnPopped != null)
					e.OnPopped(e.Value);
			}
	...
	// moved out of ContextStack, to be accessible by AOT.

	public struct ContextStackEntry<TypeT>
	{
		public TypeT Value;
